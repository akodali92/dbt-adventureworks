...
final as (
	...
	select
		...
		{{ dbt_utils.star(from=ref('dwh_dim_employee'), relation_alias='e', except=["business_entity_id", "row_guid", "national_id_number", "org_node", "org_level", "vacation_hours", "sick_leave_hours"]) }},
		{{ dbt_utils.star(from=ref('dwh_dim_shift'), relation_alias='s', except=["shift_id", "start_time", "end_time"]) }},
		{{ dbt_utils.star(from=ref('dwh_dim_department'), relation_alias='d', except=["business_entity_id", "department_id"]) }},
		{{ dbt_utils.star(from=ref('dwh_dim_person'), relation_alias='p', except=["business_entity_id", "row_guid"]) }}
